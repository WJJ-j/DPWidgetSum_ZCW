...
+ (DPTimeAlertPickerView *)alertViewForParent:(UIView *)aParent uuid:(NSInteger)aUuid alertViewStyle:(DPTimeViewStyle)aAlertViewStyle level:(NSInteger)aLevel AlertBlock:(DPTimePickerViewBlock)aAlertBlock {
    //传参错误
    if (aLevel < 1 || aLevel > 3) {
        NSLog(@"\nError: DPTimeAlertPickerView省市区层级aLevel参数错误!\n");
        return nil;
    }
    if (aParent == nil) {
