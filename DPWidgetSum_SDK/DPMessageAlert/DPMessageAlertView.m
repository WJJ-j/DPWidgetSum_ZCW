...
- (id)init {
    ...
    if (self) {
        ...
        self.backgroundColor = DPSMDate.alertMsgBG[@"bgColor"];
        ...
        _bgView.backgroundColor = DPSMDate.alertMsgCenterBG[@"bgColor"];
        _bgView.layer.masksToBounds = YES;
        _bgView.layer.cornerRadius = [DPSMDate.alertMsgCenterBG[@"radius"] floatValue];
        ...
        _titleLab.font = DPSMDate.alertMsgTitle[@"font"];
        _titleLab.textColor = DPSMDate.alertMsgTitle[@"textColor"];
        ...
        _contentLab.font = DPSMDate.alertMsgContent[@"font"];
        _contentLab.textColor = DPSMDate.alertMsgContent[@"textColor"];
        ...
        for (int i = 0; i < ButtonMaxCount; i++) {
            ...
            aBtn.contentHorizontalAlignment = UIViewAutoresizingFlexibleTopMargin;
            aBtn.layer.masksToBounds = YES;
            aBtn.backgroundColor = DPSMDate.alertMsgBtnOther[@"bgColor"];
            ...
            aBtn.titleLabel.font = DPSMDate.alertMsgBtnOther[@"font"];
            [aBtn setTitleColor:DPSMDate.alertMsgBtnOther[@"titleColor"] forState:UIControlStateNormal];
            dp_arc_block(self);
            ...
            if (i != 0) {
                ...
                btnLineView.frame = CGRectMake(0, 0, [DPSMDate.alertMsgBtnLineLeft[@"borderWidth"] floatValue], 0);
                btnLineView.tag = 1000+aBtn.tag;
                UIColor *btnColor = DPSMDate.alertMsgBtn[@"bgColor"];
                UIColor *btnColorOther = DPSMDate.alertMsgBtnOther[@"bgColor"];
                if (CGColorEqualToColor(btnColor.CGColor, btnColorOther.CGColor)) {
                    btnLineView.backgroundColor = DPSMDate.alertMsgBtnLineLeft[@"bgColor"];
                }else {
        ...
        _lineView.backgroundColor = DPSMDate.alertMsgBtnLine[@"bgColor"];
...
- (void)updateView {
    ...
    if (_currentStyle == DPMessageAlertStyleDefault) {
        //按钮顶部水平分割线
        _lineView.hidden = NO;
        _lineView.frame = CGRectMake(0, minY+DP_FrameHeight(20), _bgView.dp_width, [DPSMDate.alertMsgBtnLine[@"borderWidth"] floatValue]);
        ...
        for (int i = 0; i < ButtonMaxCount; i++) {
            ...
            if (i < _buttonTitles.count) {
                ...
                if ([_buttonTitles[i] isKindOfClass:[NSString class]]) {
                    if (i == 0) {
                        aBtn.backgroundColor = DPSMDate.alertMsgBtn[@"bgColor"];
                        [aBtn setTitleColor:DPSMDate.alertMsgBtn[@"titleColor"] forState:UIControlStateNormal];
                    }else {
                        aBtn.backgroundColor = DPSMDate.alertMsgBtnOther[@"bgColor"];
                        [aBtn setTitleColor:DPSMDate.alertMsgBtnOther[@"titleColor"] forState:UIControlStateNormal];
                    }
...
- (void)addTapContentForText:(NSString *)aText url:(NSString *)aUrl attributes:(NSDictionary *)aAttributes addIndex:(NSInteger)aAddIndex block:(BOOL (^)(DPMessageAlertView *aObject, NSString *aText, NSString *aUrl))aBlock {
    ...
    NSDictionary *pLinkAttributes = aAttributes != nil ? aAttributes : @{
        (id)kCTForegroundColorAttributeName:DPSMDate.alertMsgBtn[@"titleColor"],
