    ...
    if (aTool) {
        ...
        if (aPickerType == 0) {
            [DPPermissionsTool isOpenCaptureDeviceServiceWithBlock:^(BOOL isOpen) {
                if (!isOpen) {
                    [ZCWAlertView showDPMessageAlertViewForTitle:nil content:@"您没有开启相机权限，开启后即可拍照" buttonTitles:@[@"去开启权限"] buttonBlock:^BOOL(DPMessageAlertView *aObject, NSInteger tapIndex) {
            ...
            [DPPermissionsTool isOpenAlbumServiceWithBlock:^(BOOL isOpen) {
                if (!isOpen) {
                    [ZCWAlertView showDPMessageAlertViewForTitle:nil content:@"您没有开启相册权限，开启后即可查看相册图片" buttonTitles:@[@"去开启权限"] buttonBlock:^BOOL(DPMessageAlertView *aObject, NSInteger tapIndex) {
                        [DPPermissionsTool openApplicationSettingsWithBlock:nil];
                        return YES;
                    }];
                }else {
