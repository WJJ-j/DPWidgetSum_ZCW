...
@property (nonatomic, assign) DPCustomBannerDirection direction;
...
- (instancetype)initWithFrame:(CGRect)frame currentPage:(NSInteger)currentPage showDataArr:(NSArray*)dataArray timeInterval:(NSTimeInterval)timeInterval direction:(DPCustomBannerDirection)direction tag:(NSInteger)tag delegate:(id<DPCustomBannerDelegate>)delegate{
    ...
    self=[[DPCustomBanner alloc]initWithFrame:frame placeholderBGImage:nil currentPage:currentPage showDataArr:dataArray timeInterval:timeInterval direction:direction tag:tag delegate:delegate];
    return self;
}
...
- (void)setScrollViewData:(NSArray*)dataArray{
    ...
    if (_dataArr.count==1) {
        self.singleView=[self.delegate customBannerChildViewWithFrame:self.bounds data:_dataArr[0] tag:self.tag index:0];
    ...
    if (self.direction == DPCustomBannerLandscape) {
        ...
        if (!_pageC) {
            ...
            _pageC.enabled = NO;
            _pageC.currentPageIndicatorTintColor = DP_RGBA(0, 0, 0, 1);
            _pageC.pageIndicatorTintColor = DP_RGBA(0, 0, 0, 0.2);
    ...
    }else if (self.direction == DPCustomBannerPortrait){
...
//    NSLog(@"%ld %d %d",(long)pre,_currentPage,last);
    ...
    if ([self.delegate respondsToSelector:@selector(customBannerChildViewWithFrame:data:tag:index:)]) {
        UIView* leftView=[self.delegate customBannerChildViewWithFrame:self.bounds data:_dataArr[pre] tag:self.tag index:pre];
        leftView.tag=pre;
        
        UIView* midView=[self.delegate customBannerChildViewWithFrame:self.bounds data:_dataArr[_currentPage] tag:self.tag index:_currentPage];
        midView.tag=_currentPage;

        UIView* rightView=[self.delegate customBannerChildViewWithFrame:self.bounds data:_dataArr[last] tag:self.tag index:last];
        rightView.tag=last;
...
-(void)handleTap:(UITapGestureRecognizer*)tap{
    if ([self.delegate respondsToSelector:@selector(customBannerDidSelectedViewIndex:data:tag:)]) {
        [self.delegate customBannerDidSelectedViewIndex:tap.view.tag data:_dataArr[tap.view.tag] tag:self.tag];
    }
}
