...
//  ZCWBaseApp
//
//  Created by dapengmac on 15/11/16.
...
//回调
@property (nonatomic, copy) DPAddressPickerViewBlock aAlertBlock;
//弹框父视图
...
- (void)reloadPickerViewDataModel {
    ...
    if (_level > 2) {
        NSInteger row = 0;
        for (int i = 0; i < _aCityModel.arr.count; i++) {
            if (_aCountyModel.countyCode.integerValue == _aCityModel.arr[i].countyCode.integerValue) {
                row = i;
                break;
            }
        }
...
- (void)showTypeAlertViewForIsShow:(BOOL)aIsShow updateDataBlock:(NSArray <DPProvinceModel *>* (^)(id aData))aBlock {
    dp_arc_block(self);
    if (aIsShow) {
        if (aBlock) {
            self.aProvinceArray = aBlock(nil);
        }else if (_aProvinceArray.count < 1) {
...
- (void)setAlertViewForProvince:(NSString *)aProvince city:(NSString *)aCity county:(NSString *)aCounty isAlertBlock:(BOOL)aIsAlertBlock {
    ...
    if (aCounty.length) {
        for (int i = 0; i < _selectCityModel.arr.count; i++) {
            if (aCounty.integerValue == _selectCityModel.arr[i].countyCode.integerValue) {
                self.selectCountyModel = _selectCityModel.arr[i];
            }
        }
    }
...
- (NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    ...
    if (component == 0) {
        ...
        str = dp_notEmptyStr(_aCityModel.arr[row].county);
    }
    return str;
}
//选择行数据
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    switch (component) {
        case 0:{
            ...
            if (_level > 2) {
                self.aCountyModel = [_aCityModel.arr dp_objectAtIndex:0];
        ...
        case 1:{
            ...
            break;
        case 2:{
            self.aCountyModel = [_aCityModel.arr dp_objectAtIndex:row];
        }
            break;
        default:
            break;
...
- (NSArray <DPCountyModel *>*)getCountyArray{
    ...
    for (DPCityModel *bModel in aCityArray) {
        if ([bModel.cityCode isEqualToString:_aCityModel.cityCode]) {
            NSMutableArray *tempArray = [NSMutableArray arrayWithCapacity:bModel.arr.count];
            for (DPCountyModel *cModel in bModel.arr) {
...
#pragma mark <--------------外部调用函数-------------->
+ (DPAddressAlertPickerView *)alertViewForParent:(UIView *)aParent uuid:(NSInteger)aUuid alertViewStyle:(DPAddressViewStyle)aAlertViewStyle level:(NSInteger)aLevel alertBlock:(DPAddressPickerViewBlock)aAlertBlock {
    //传参错误
...
+ (void)showTypeAlertViewForParent:(UIView *)aParent uuid:(NSInteger)aUuid isShow:(BOOL)aIsShow updateDataBlock:(NSArray <DPProvinceModel *>* (^)(id aData))aBlock {
    for (UIView *aView in aParent.subviews) {
        if ([aView isKindOfClass:[DPAddressAlertPickerView class]] && aView.tag == DPAddressAlertPickerViewTag+aUuid) {
            [((DPAddressAlertPickerView *)aView) showTypeAlertViewForIsShow:aIsShow updateDataBlock:aBlock];
        }
    }
}

//根据省、市、区(县)code码获得拼接地址
+ (void)addressSumForProvinceCode:(NSString *)aProvinceCode cityCode:(NSString *)aCityCode countyCode:(NSString *)aCountyCode updateDataBlock:(NSArray <DPProvinceModel *>* (^)(id aData))aBlock callBackBlock:(void (^)(NSString *address))callBackBlock {
    if (callBackBlock) {
        NSString *str = @"";
        
        NSArray <DPProvinceModel *>* aProvinceArray = [NSArray array];
        if (aBlock) {
            aProvinceArray = aBlock(nil);
        }
        ...
        if (aProvinceArray.count > 0) {
            ...
            for (DPProvinceModel *aModel in aProvinceArray) {
                if ([aModel.proCode isEqualToString:aProvinceCode]) {
                    str = [str stringByAppendingString:aModel.province];
            ...
            for (DPCityModel *aModel in aCityArray) {
                if ([aModel.cityCode isEqualToString:aCityCode]) {
                    str = [str stringByAppendingString:aModel.city];
                    aCountyArray = aModel.arr;
                }
            }
            
            for (DPCountyModel *aModel in aCountyArray) {
                if ([aModel.countyCode isEqualToString:aCountyCode]) {
                    str = [str stringByAppendingString:aModel.county];
                }
            }
        }
