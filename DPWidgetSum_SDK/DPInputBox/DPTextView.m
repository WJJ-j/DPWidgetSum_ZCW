...
- (id)initWithFrame:(CGRect)aFrame delegate:(UIViewController *)aDelegate superView:(UIView *)aSuperView {
    ...
    if (self) {
        ...
        _textViewType = DPTextViewTypeNormal;
...
- (void)setTextViewType:(DPTextViewType)TextViewType {
...
//刷新输入框输入限制
- (void)loadTextLimit {
    if (_textViewType == DPTextViewTypeNormal) {
        self.textLimitType = 0;
        self.textLengthMax = 200;
        self.textRegular = @"";
        self.textFormatRegular = @[];
        
        _aTextView.returnKeyType = UIReturnKeyDone;
    }
}
//刷新页面布局
- (void)loadViewLayout {
    ...
    if (tempText.length > 0) {
        if (_textLimitType == 0) {
            ...
            if ([DPTextInputTool byteCountWithStr:tempText] > _textLengthMax) {
                ...
                [DPTool showToastMsg:_textMaxMessage forView:_aSuperView];
...
- (void)updateCountLabText {
    ...
    _countLab.text = [NSString stringWithFormat:@"%ld/%ld",(long)currentCount,(long)_textLengthMax];
}
@end
