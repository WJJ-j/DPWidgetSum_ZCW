...
typedef NS_ENUM(NSInteger, DPTextFieldType) {
    //无属性
    DPTextFieldTypeNormal = 0,
    //账户名
    DPTextFieldTypeName = 1,
    //手机号
    DPTextFieldTypeCell = 2,
    //密码,限制系统键盘
    DPTextFieldTypePassword = 3,
    //密码,不限制键盘类型 和 输入类型
    DPTextFieldTypePasswordChinese = 4,
    //验证码
    DPTextFieldTypeCode = 5
};
...
///输入框输入最大长度，默认：60；
@property (nonatomic, assign) NSInteger textLengthMax;
///输入框输入长度超限提示语，默认：最多输入n个字符 或 最多输入n个字；
